...
def estimateGaussianParams(tdata,xn):
...
def analyseStellarSpectra(stellar_ccf_G2):
	...
	mjd=np.empty(len(stellar_ccf_G2))

	for i in range(0,len(stellar_ccf_G2)):
		h=fits.open(stellar_ccf_G2[i])
		...
		fig.clf()
		fig.suptitle('%s CCFs' % (stellar_ccf_G2[i]))
		...
		fig.savefig('%s.png' % (stellar_ccf_G2[i]),dpi=300)

	# plot the results from the analysis of the RVs/RM
	mjds=mjd-int(mjd[0])
	correction=np.average(rv)-gamma_vel
	rv_corr=rv-correction
	fig = pl.figure(i+1,figsize=(15,15))
	fig.clf()
	ax = fig.add_subplot(3,1,1)
	ax.plot(mjds,rv_corr,'ro')
